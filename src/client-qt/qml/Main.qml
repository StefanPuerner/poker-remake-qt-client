...
ApplicationWindow {
    ...
    // Herramienta de desarrollo (--demo-local, ver main.cpp): la partida local se juega sola.
    property bool demoAuto: false
    property bool demoNoVota: false
    // Herramienta de desarrollo (--llamar iniciarDemoRanking): abre el Ranking y, un momento después,
    // simula la llegada de los datos del servidor, para comprobar el scroll inicial sin servidor.
    function iniciarDemoRanking() {
        ventana.entrarSinConexion(false);
        ventana.pantalla = "Ranking";
        demoRankingDatos.start();
    }
    Timer {
        id: demoRankingDatos
        interval: 600
        onTriggered: {
            var filas = [];
            for (var i = 0; i < 14; i++) {
                filas.push({ accountId: i + 1, partidasJugadas: 40 + i, partidasGanadas: 30 - i, elo: 1500 - i * 20,
                             tieneMarcoBasico: true, textura: "", efecto: "", decoracionLateral1: "", decoracionLateral2: "",
                             decoracionSuperior: "", acabadoLateral1: "", acabadoLateral2: "", acabadoSuperior: "",
                             titulo: "", username: "Jugador" + (i + 1) });
            }
    ...
    // Sonda de desarrollo: escribe dónde ha quedado el scroll del ranking (console.warn sí sale en release).
    Timer {
        id: sondaRanking
        interval: 1500
        onTriggered: console.warn("RANKING contentY=" + listaRanking.contentY + " originY=" + listaRanking.originY
    ...
    // ── Sonidos y animaciones de la mesa (docs/plan-integracion-animaciones.md) ──────────────
    ...
    function sonar(evento, opciones) {
        if (!ventana.sonidoActivado || ventana.grupoSilenciado(ventana.grupoDeEvento[evento])) return;
        bancoSonidos.programar(ventana.configSonidos[evento], opciones);
    }
    // Carga eventos.json (recurso) y prepara las rutas de los WAV para las voces del banco.
    function cargarConfigSonidos() {
        try {
            var cfg = JSON.parse(recursos.leerTexto("qrc:/qt/qml/PokerQuick/assets/sonidos/eventos.json"));
            var rutas = {};
            for (var ev in cfg)
                for (var l in cfg[ev])
                    cfg[ev][l].sonidos.forEach(function(n) {
                        rutas[n] = "qrc:/qt/qml/PokerQuick/assets/sonidos/mesa/" + n + ".wav";
                    });
            ventana.configSonidos = cfg;
            bancoSonidos.rutas = rutas;
        } catch (e) {
    ...
    function reordenarRanking() {
        ...
        Qt.callLater(() => { listaRanking.contentY = listaRanking.originY; });
    }
    // ── Social ───────────────────────────────────────────────────────────
    ListModel {
        id: modeloAmigos
    ...
    readonly property bool votoConPlazo: votoPlazoSegundos > 0
    property double votoInicioMs: 0
    property int votoRestanteSegundos: 60
    Timer {
        interval: 1000
        repeat: true
        running: ventana.votoAbierto && !ventana.modoOfflineActivo && ventana.votoConPlazo
    ...
    // Tus dos cartas de la barra: retenidas boca abajo hasta que la carta voladora aterriza, y
    // ocultas cuando vuelven al dealer/mazo (fin de mano o retirada).
    ...
    // Posición de tus dos huecos de carta (barra inferior) en coordenadas de la mesa: a ellos
    // aterrizan las cartas del reparto y de ellos salen al retirarte o al acabar la mano.
    function slotsPropiosEnMesa() {
        var r = [];
        [cartaPropia1, cartaPropia2].forEach(function(c) {
            var p = mesaJuego.mapFromItem(c, 0, 0);
            r.push({ x: p.x, y: p.y, w: c.width, h: c.height });
        });
        return r;
    }
    ...
    // Los dos números de "X / Y listos" (Lobby) -- una property normal en
    // vez de asignar contadorListos.text directamente, para que el texto
    // se re-traduzca solo si cambia el idioma (ver onLobbyActualizado).
    ...
    Rectangle {
        ...
        Column {
            ...
            BotonContorno {
                ...
                onClicked: {
                    ...
                    nombreUsuario.text = Idioma.t("prefijo_invitado") + Math.floor(Math.random() * 100000);
                    pantalla = "Salas";
        ...
        Column {
            ...
            Item {
                ...
                Text {
                    ...
                    anchors.left: parent.left
                    anchors.verticalCenter: parent.verticalCenter
                    text: viendoGuardadas ? Idioma.t("titulo_partidas_guardadas") : Idioma.t("titulo_salas_disponibles")
                ...
                Row {
                    ...
                    CampoTexto {
                        ...
                        placeholderText: (activeFocus || text.length > 0) ? "" : Idioma.t("placeholder_codigo_sala")
                    ...
                    BotonContorno {
                        ...
                        anchors.verticalCenter: parent.verticalCenter
                        text: Idioma.t("boton_unirse_codigo")
                        onClicked: {
            ...
            Row {
                ...
                SelectorSegmentado {
                    ...
                    opciones: [Idioma.t("tab_salas"), Idioma.t("titulo_partidas_guardadas")]
                ...
                delegate: Rectangle {
                    ...
                    Column {
                        ...
                        Text {
                            ...
                            text: Idioma.tf("guardada_detalle",
                                            [filaGuardada.fecha, filaGuardada.humanos, filaGuardada.bots])
                            color: Tema.colorTextoTenue
                            font.pixelSize: 12 * Tema.escala
                        }
                    }
                    ...
                    Row {
                        ...
                        BotonContorno {
                            anchors.verticalCenter: parent.verticalCenter
                            height: filaAcciones.altoBoton
                            text: filaGuardada.confirmandoBorrado ? Idioma.t("boton_confirmar_borrado") : "🗑"
                            colorBorde: Tema.colorPeligro
                            onClicked: {
        ...
        Column {
            ...
            Text {
                anchors.horizontalCenter: parent.horizontalCenter
                text: Idioma.t("titulo_ranking_global")
                color: Tema.colorTexto
                font.family: Tema.fuenteElegante
            ...
            Row {
                ...
                Text {
                    anchors.verticalCenter: parent.verticalCenter
                    text: Idioma.tf("ranking_subtitulo",
                              [ordenRankingActual === 0 ? Idioma.t("orden_victorias")
                                  : ordenRankingActual === 1 ? Idioma.t("orden_ratio") : Idioma.t("orden_elo")])
                    color: Tema.colorTextoMuyTenue
                    font.pixelSize: 11 * Tema.escala
                }
        ...
        Rectangle {
            ...
            ListView {
                ...
                // CAUSA REAL (medida el 2026-09-20 con --llamar iniciarDemoRanking: contentY=0,
                // originY=-373.8, cabecera=373.8): con un `header`, el INICIO del contenido de un
                // ListView no está en contentY=0 sino en `originY` (= -alto de la cabecera). Los
                // cinco intentos anteriores fijaban contentY = 0, que es justo pasar de largo el
                // podio (la cabecera queda por encima, en negativo). El inicio es `originY`.
                ...
                header: Column {
                    ...
                    onHeightChanged: Qt.callLater(() => { listaRanking.contentY = listaRanking.originY; })
                            ...
                            delegate: Column {
                                ...
                                Text {
                                    ...
                                    text: !columnaPodio.fila ? ""
                                          : ordenRankingActual === 2 ? Idioma.tf("podio_stat_elo", [columnaPodio.fila.elo])
                                          : ordenRankingActual === 1
                                            ? Idioma.tf("podio_stat_ratio",
                                                  [Math.round(100 * columnaPodio.fila.partidasGanadas / columnaPodio.fila.partidasJugadas)])
                                            : Idioma.tf("podio_stat_victorias", [columnaPodio.fila.partidasGanadas])
                                    color: Tema.colorTextoTenue
                                    font.pixelSize: 11 * Tema.escala
                                }
                            }
                    ...
                    Row {
                        ...
                        Text {
                            ...
                            text: ordenRankingActual === 2 ? Idioma.t("header_elo") : Idioma.t("header_ganadas")
                            color: Tema.colorTextoMuyTenue
                            font.pixelSize: 10 * Tema.escala
                        }
                        Text {
                            width: 110 * Tema.escala
                            horizontalAlignment: Text.AlignRight
                            text: ordenRankingActual === 2 ? Idioma.t("header_partidas") : Idioma.t("header_ratio")
                            color: Tema.colorTextoMuyTenue
                            font.pixelSize: 10 * Tema.escala
                        }
                    }
        ...
        Column {
            ...
            SelectorSegmentado {
                id: tabsSocial
                width: parent.width
                opciones: [Idioma.t("tab_amigos"), Idioma.t("tab_buscar_jugadores"),
                           Idioma.t("tab_jugadores_recientes"), Idioma.t("tab_solicitudes")]
        ...
        Item {
            ...
            Rectangle {
                ...
                Text {
                    anchors.centerIn: parent
                    width: parent.width - 60 * Tema.escala
                    text: Idioma.tf("vacio_amigos", [Idioma.t("tab_buscar_jugadores"), Idioma.t("tab_jugadores_recientes")])
                    color: Tema.colorTextoTenue
                    horizontalAlignment: Text.AlignHCenter
                    wrapMode: Text.WordWrap
                    font.pixelSize: 13 * Tema.escala
            ...
            Row {
                    ...
                    delegate: Item {
                        ...
                        Column {
                            ...
                            Text {
                                ...
                                text: filaAmigoChat.ultimoTexto !== ""
                                          ? (filaAmigoChat.ultimoEsMio ? Idioma.t("prefijo_tu_dos_puntos") : "") + filaAmigoChat.ultimoTexto
                        ...
                        Column {
                            ...
                            Row {
                                ...
                                Text {
                                    anchors.verticalCenter: parent.verticalCenter
                                    text: filaAmigoChat.estado === "CONECTADO" ? Idioma.t("estado_conectado")
                                          : filaAmigoChat.estado === "EN_PARTIDA" ? Idioma.t("estado_en_partida")
                ...
                Rectangle {
                    ...
                    Column {
                        ...
                        Text {
                            anchors.horizontalCenter: parent.horizontalCenter
                            text: Idioma.t("chat_elige_conversacion")
                            color: Tema.colorTextoMuyTenue
                            font.pixelSize: 13 * Tema.escala
                        }
                    }
                        ...
                        Row {
                            ...
                            Column {
                                ...
                                Row {
                                    ...
                                    Text {
                                        anchors.verticalCenter: parent.verticalCenter
                                        text: chatAmigoSeleccionadoEstado === "CONECTADO" ? Idioma.t("estado_conectado")
                                              : chatAmigoSeleccionadoEstado === "EN_PARTIDA" ? Idioma.t("estado_en_partida")
                                              : Idioma.t("estado_desconectado")
                                        color: Tema.colorTextoTenue
                                        font.pixelSize: 11 * Tema.escala
                                    }
            ...
            Item {
                ...
                Row {
                    ...
                    CampoTexto {
                        ...
                        anchors.verticalCenter: parent.verticalCenter
                        width: 260 * Tema.escala
                        placeholderText: (activeFocus || text.length > 0) ? "" : Idioma.t("placeholder_nombre_usuario")
                        font.pixelSize: 13 * Tema.escala
                ...
                ListView {
                    ...
                    delegate: Rectangle {
                        ...
                        BotonContorno {
                            ...
                            text: filaBusqueda.solicitudEnviada ? Idioma.t("estado_solicitud_enviada") : Idioma.t("boton_enviar_solicitud")
                            onClicked: {
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    BotonContorno {
                        ...
                        text: filaReciente.solicitudEnviada ? Idioma.t("estado_solicitud_enviada") : Idioma.t("boton_enviar_solicitud")
                        onClicked: {
        ...
        BarraSuperior {
            ...
            anchors.left: parent.left
            anchors.right: parent.right
            textoCentro: Idioma.t("boton_crear_sala")
            pantalla: ventana.pantalla
            miSaldoActual: ventana.miSaldoActual
            reconectandoAhora: ventana.reconectandoAhora
            servidorHost: ventana.servidorHost
        ...
        Item {
            ...
            Rectangle {
                ...
                Column {
                    ...
                    Text {
                        anchors.horizontalCenter: parent.horizontalCenter
                        text: partidaGuardada ? Idioma.t("titulo_partida_guardada") : Idioma.t("titulo_partida_finalizada")
                        color: Tema.colorAccent
                        font.letterSpacing: 2
                        font.pixelSize: 12 * Tema.escala
                        font.bold: true
                    ...
                    Text {
                        visible: !partidaGuardada
                        anchors.horizontalCenter: parent.horizontalCenter
                        text: Idioma.tf("etiqueta_fichas", [saldoFinal])
                        color: Tema.colorAccent
                        font.pixelSize: 15 * Tema.escala
                    }
                    Text {
                        ...
                        width: parent.width
                        wrapMode: Text.WordWrap
                        text: finPorLimite ? Idioma.t("texto_fin_por_limite") : Idioma.t("texto_fin_por_eliminacion")
                        color: Tema.colorTextoTenue
                        font.pixelSize: 12 * Tema.escala
                    }
                    ...
                    Text {
                        anchors.horizontalCenter: parent.horizontalCenter
                        visible: !partidaGuardada
                        text: Idioma.tf("etiqueta_manos_disputadas", [manosDisputadasFinal])
                        color: Tema.colorTextoTenue
                        font.pixelSize: 12 * Tema.escala
                    }
                    Column {
        ...
        BarraSuperior {
            ...
            anchors.left: parent.left
            anchors.right: parent.right
            textoCentro: hostActual !== "" ? Idioma.tf("titulo_sala_de", [hostActual]) : ""
            pantalla: ventana.pantalla
            miSaldoActual: ventana.miSaldoActual
            reconectandoAhora: ventana.reconectandoAhora
            servidorHost: ventana.servidorHost
        ...
        Item {
            ...
            Column {
                ...
                Column {
                    spacing: 2 * Tema.escala
                    Text {
                        text: hostActual !== "" ? Idioma.tf("titulo_sala_de", [hostActual]) : Idioma.t("titulo_sala_espera")
                        color: Tema.colorTexto
                        font.family: Tema.fuenteElegante
                        font.pixelSize: 22 * Tema.escala
                        font.bold: true
                    ...
                    Text {
                        ...
                        // binding lo sustituye; sin la guarda se vería
                        // "0 / 0 listos" un instante antes de tener datos.
                        visible: esperadosActuales > 0
                        text: Idioma.tf("etiqueta_listos", [listosActuales, esperadosActuales])
                        color: Tema.colorTextoTenue
                        font.pixelSize: 12 * Tema.escala
                    }
                    Row {
                        ...
                        Text {
                            anchors.verticalCenter: parent.verticalCenter
                            text: Idioma.tf("etiqueta_codigo_invitar", [codigoSalaPropia])
                            color: Tema.colorAccent
                            font.pixelSize: 12 * Tema.escala
                            font.bold: true
                        }
                    ...
                    Text {
                        ...
                        text: Idioma.tf("etiqueta_nombres_esperados", [nombresEsperadosLobby])
                        color: Tema.colorTextoTenue
                        font.pixelSize: 12 * Tema.escala
                        wrapMode: Text.WordWrap
                        width: 260 * Tema.escala
                ...
                Text {
                    ...
                    text: Idioma.tf("etiqueta_esperando_sentarse", [listaEsperando.join(", ")])
                    color: Tema.colorTextoTenue
                    font.pixelSize: 12 * Tema.escala
                    wrapMode: Text.WordWrap
                    width: 260 * Tema.escala
        ...
        Item {
            ...
            BarraSuperior {
                ...
                textoCentro: Idioma.tf("etiqueta_estado_mesa",
                                       [rondaActual, manoActual, Math.round(ciegaActual / 2), ciegaActual, turnoNombre])
                pantalla: ventana.pantalla
                miSaldoActual: ventana.miSaldoActual
                reconectandoAhora: ventana.reconectandoAhora
                servidorHost: ventana.servidorHost
            ...
            Popup {
                ...
                contentItem: ScrollView {
                    ...
                    Column {
                        ...
                        Text {
                            width: parent.width
                            text: Idioma.t("titulo_ranking_manos")
                            color: Tema.colorTexto
                            font.family: Tema.fuenteElegante
                            font.bold: true
                            font.pixelSize: 16 * Tema.escala
                        ...
                        Repeater {
                            model: [
                                { nombre: "1. " + Idioma.t("combo_escalera_real"), cartas: ["AS", "KS", "QS", "JS", "TS"] },
                                { nombre: "2. " + Idioma.t("combo_escalera_color"), cartas: ["9H", "8H", "7H", "6H", "5H"] },
                                { nombre: "3. " + Idioma.t("combo_poker"), cartas: ["KS", "KH", "KD", "KC", "5S"] },
                                { nombre: "4. " + Idioma.t("combo_full_house"), cartas: ["JS", "JH", "JD", "4C", "4S"] },
                                { nombre: "5. " + Idioma.t("combo_color"), cartas: ["AC", "JC", "8C", "6C", "2C"] },
                                { nombre: "6. " + Idioma.t("combo_escalera"), cartas: ["TS", "9H", "8D", "7C", "6S"] },
                                { nombre: "7. " + Idioma.t("combo_trio"), cartas: ["7S", "7H", "7D", "KC", "2S"] },
                                { nombre: "8. " + Idioma.t("combo_doble_pareja"), cartas: ["QS", "QH", "9D", "9C", "4S"] },
                                { nombre: "9. " + Idioma.t("combo_pareja"), cartas: ["TS", "TH", "KD", "6C", "2S"] },
                                { nombre: "10. " + Idioma.t("combo_carta_alta"), cartas: ["AS", "JH", "8D", "6C", "3S"] }
                            ]
                            delegate: Column {
                                required property var modelData
                                required property int index
            ...
            Item {
                ...
                Mesa {
                    ...
                    // Fin de reparto/aviso de la carta propia y sonidos de la mesa.
                    ...
                    onRepartoTerminado: {
                        ventana.retenerMano1 = false;
                        ventana.retenerMano2 = false;
                    }
                    onCartaPropiaAterrizada: function(indice) {
                        if (indice === 0) ventana.retenerMano1 = false; else ventana.retenerMano2 = false;
                    }
            ...
            // ── "Fin de la mano": tarjeta centrada sobre la mesa mientras se decide (seguir/abandonar/
            // guardar). En red, con la cuenta atrás del plazo del voto (el servidor sigue solo al vencer).
            Item {
                id: tarjetaFinDeMano
                visible: votoAbierto && !tuTurno
                z: 25
                anchors.horizontalCenter: parent.horizontalCenter
                anchors.top: barraSuperior.bottom
                anchors.topMargin: 8 * Tema.escala
                width: Math.max(textoFinDeMano.implicitWidth, textoCuentaAtras.implicitWidth) + 96 * Tema.escala
                height: (ventana.modoOfflineActivo || !ventana.votoConPlazo ? 46 : 62) * Tema.escala
                ...
                Rectangle {
                    ...
                    Column {
                        ...
                        Text {
                            id: textoCuentaAtras
                            visible: !ventana.modoOfflineActivo && ventana.votoConPlazo
                            anchors.horizontalCenter: parent.horizontalCenter
                            text: Idioma.tf("voto_continua_solo", [ventana.votoRestanteSegundos])
                            color: Tema.colorTextoTenue
                            font.family: Tema.fuenteElegante
                            font.pixelSize: 11 * Tema.escala
                        }
            ...
            Rectangle {
                ...
                Row {
                    ...
                    Column {
                        spacing: 2 * Tema.escala
                        Text {
                            text: Idioma.t("etiqueta_combo_actual")
                            color: Tema.colorTextoTenue
                            font.pixelSize: 10 * Tema.escala
                        }
                        Text {
                    ...
                    Column {
                        spacing: 2 * Tema.escala
                        Text {
                            text: Idioma.t("etiqueta_combo_probable")
                            color: Tema.colorTextoTenue
                            font.pixelSize: 10 * Tema.escala
                        }
                        Text {
                    ...
                    Column {
                        spacing: 2 * Tema.escala
                        Text {
                            text: Idioma.t("etiqueta_combo_maxima")
                            color: Tema.colorTextoTenue
                            font.pixelSize: 10 * Tema.escala
                        }
                        Text {
                ...
                Row {
                    ...
                    BotonContorno {
                        text: aPagarParaIgualar > 0 ? Idioma.tf("boton_igualar", [aPagarParaIgualar]) : Idioma.t("boton_pasar")
                        onClicked: {
                ...
                Row {
                    ...
                    Text {
                        anchors.verticalCenter: parent.verticalCenter
                        text: Idioma.t("texto_sin_fichas")
                        color: Tema.colorTextoTenue
                        font.pixelSize: 13 * Tema.escala
                    }
                    BotonRelleno {
                        text: recompraSolicitada ? Idioma.t("texto_recompra_enviada") : Idioma.t("boton_recomprar")
                ...
                // ── Decisiones de fin de mano, EN EL HUECO de los botones de turno: seguir /
                // abandonar / guardar (PanelVoto), extender la partida y elegir manos extra. Y, si
                // te retiraste o ganaste sin showdown, "Mostrar cartas" (opcional: no influye en
                // el resultado). Todo mientras la mesa sigue a la vista.
                Row {
                    id: decisionesFin
                    ...
                    BotonContorno {
                        // Solo si tu mano NO está ya revelada (por obligación en el showdown o por haberla enseñado).
                        visible: ventana.puedeMostrarMisCartas && votoAbierto && !mesaJuego.muestras[nombreUsuario.text]
                        anchors.verticalCenter: parent.verticalCenter
                ...
                // El título "Fin de la mano" y la cuenta atrás viven en una tarjeta sobre la mesa
                // (ver tarjetaFinDeMano); aquí solo los botones.
                PanelVoto {
                    visible: votoAbierto
                    anchors.verticalCenter: parent.verticalCenter
                    // Mismo bug de scope que la barra superior (ver comentario en la instanciación de
                    // "Salas" más arriba) -- PanelVoto declara su propio "soyHost", así que bajo
                    // ComponentBehavior: Bound la referencia suelta se autorreferenciaba.
        ...
        Connections {
            ...
            function onFichasApostadas(jugador, cantidad) {
                if (ventana.nivelAnimaciones >= 1) mesaJuego.lanzarFichas(jugador, cantidad);
            }
            ...
            function onJugadorAllIn(jugador, cantidad, porCiega) {
                if (ventana.nivelAnimaciones >= 1) mesaJuego.avisarAllIn(jugador, cantidad, !porCiega);
            }
            ...
            function onJugadoresEliminados(jugadoresCsv) {
                ...
                if (ventana.nivelAnimaciones < 1) {
                    mesaJuego.eliminados = mesaJuego.eliminados.concat(nombres);
                    return;
                }
            ...
            function onOrdenShowdown(jugadoresCsv) {
                ...
                tuTurno = false;
                turnoNombre = "";
                mesaJuego.prepararShowdown(jugadoresCsv.length > 0 ? jugadoresCsv.split(",") : []);
            }
            function onManoRevelada(jugador, cartasCsv, combo, mejoresCsv, temprana) {
                ...
                if (jugador === nombreUsuario.text) ventana.puedeMostrarMisCartas = false;
                mesaJuego.revelarMano(jugador, cartasCsv.split(","), comboLocalizado(combo));
            ...
            function onDetalleBote(numBote, aportesCsv) {
                ...
                var d = Object.assign({}, ventana.detallesBotes);
                d[numBote] = lista;
            ...
            function onEstadoMesaActualizado(ronda, bote, turno, jugadoresStr, timeoutMs,
                ...
                for (var i = 0; i < jugadores.length; i++) {
                    ...
                    var esYoLocal = ventana.modoOfflineActivo && campos[0] === nombreUsuario.text;
                ...
                if (mesaJuego.eliminados.length > 0) {
                    var apagados = [];
                    for (var e = 0; e < mesaJuego.eliminados.length; e++) {
                        for (var q = 0; q < jugadoresPartida.count; q++) {
                            var jq = jugadoresPartida.get(q);
                            if (jq.nombre === mesaJuego.eliminados[e] && (parseInt(jq.saldo) || 0) === 0) apagados.push(jq.nombre);
                        }
                    }
                    if (apagados.length !== mesaJuego.eliminados.length) mesaJuego.eliminados = apagados;
                }
            }
            ...
            function onNuevaMano(mano, ciega) {
                ...
                // sigue adelante si vence el plazo del voto). Antes lo cerraba el overlay del showdown.
                votoAbierto = false;
                votoExtensionAbierto = false;
                // Fin de la mano anterior: TODAS las cartas vuelven al dealer/mazo (las de los
                // asientos, las tuyas y las comunitarias, que primero se voltean). Sin animaciones,
                // simplemente se limpia la mesa.
                if (ventana.animarMesa) {
                    mesaJuego.slotsPropios = ventana.slotsPropiosEnMesa();
                    mesaJuego.recogerCartas();
                    ventana.miManoPresente = false;
                } else {
                    mesaJuego.reiniciarSinAnimar();
                    ventana.miManoPresente = true;
                }
                ventana.retenerMano1 = false;
                ventana.retenerMano2 = false;
                revealsShowdown = [];
            ...
            function onShowdownIniciado(cartasCsv) {
                ...
                mesaJuego.botesShowdown = [];
                mesaJuego.botePulsado = -1;
            }
            ...
            function onBoteEvaluado(numBote, cantidad, jugadoresCsv) {
                ...
                mesaJuego.botesShowdown = mesaJuego.botesShowdown.concat([{
                    numBote: numBote,
                    cantidad: cantidad,
                    ganador: "",
                    premio: 0,
            ...
            function onBoteGanado(jugador, premio, numBote, combo) {
                ...
                var botes = mesaJuego.botesShowdown.slice();
                ...
                mesaJuego.botesShowdown = botes;
                // Resaltar la mano ganadora y anunciarlo (banda, fichas al ganador, sonido).
                mesaJuego.marcarGanador(jugador, mejoresPorJugador[jugador] || []);
                mesaJuego.anunciarGanador(jugador, premio, retirados.indexOf(nombreUsuario.text) < 0,
            ...
            function onGanadorSinShowdown(jugador, bote) {
                ...
                mesaJuego.anunciarGanador(jugador, bote, retirados.indexOf(nombreUsuario.text) < 0, "");
            }
            function onMisCartasRepartidas(c1, c2) {
                // Llega justo al repartir, antes de cualquier turno. Con animaciones, tus dos
                // cartas esperan boca abajo a que aterrice cada carta voladora y entonces se voltean.
                miCarta1 = c1;
                miCarta2 = c2;
                ventana.miManoPresente = true;
                ventana.retenerMano1 = false;
                ventana.retenerMano2 = false;
                if (ventana.animarMesa) {
                    ventana.retenerMano1 = true;
                    ventana.retenerMano2 = true;
                    mesaJuego.slotsPropios = ventana.slotsPropiosEnMesa();
                    if (!mesaJuego.repartirMano(ventana.ordenRepartoActual())) {
                        ventana.retenerMano1 = false;
                        ventana.retenerMano2 = false;
                    }
                }
            ...
            function onAccionRealizada(jugador, accion) {
                if (accion === "FOLD") {
                    // Las cartas de los demás vuelan al dealer/mazo al retirarse. Las TUYAS se quedan: la
                    // estimación de combinaciones sigue actualizándose y te interesa ver qué habrías tenido.
                    if (ventana.animarMesa && jugador !== nombreUsuario.text) mesaJuego.retirarse(jugador);
            ...
            function onEsMiTurno(bote, igualar, miSaldo, miApuesta, timeoutMs, minSubida, maxSubida, c1, c2, comboA, comboP, comboM) {
                ...
                miCarta2 = c2;
                tuTurno = true;
                if (sonidoActivado && !grupoSilenciado("turno")) sonidoTurno.play();
        ...
        // Anillo de nivel/XP siempre a la vista en Cuenta (arriba a la izquierda,
        // donde sobra hueco): antes solo estaba dentro de la pestaña Progreso.
        AnilloNivel {
            visible: ventana.pantalla === "Cuenta" && hayCuenta
        ...
        Flickable {
            ...
            Column {
                ...
                Text {
                    ...
                    horizontalAlignment: Text.AlignHCenter
                    wrapMode: Text.WordWrap
                    text: Idioma.t("cuenta_invitado_aviso")
                    color: Tema.colorTextoTenue
                    font.pixelSize: 13 * Tema.escala
                }
                Row {
            ...
            Column {
                    ...
                    Column {
                        ...
                        Text {
                            text: nombreMarco(Tema.marcoPorPartidasGanadas(statsPartidasGanadas, statsTieneMarcoBasico)) +
                                  (statsPartidasJugadas > 0 ? " · " + Idioma.tf("sufijo_stat_partidas_ganadas", [statsPartidasGanadas]) : "")
                            color: Tema.colorTextoTenue
                            font.pixelSize: 13 * Tema.escala
                        }
                ...
                Row {
                    ...
                    Column {
                        ...
                        Repeater {
                            model: [
                                { etiqueta: Idioma.t("stat_partidas_jugadas"), valor: statsPartidasJugadas + "" },
                                { etiqueta: Idioma.t("stat_partidas_oficiales_ganadas"), valor: statsPartidasGanadas + "" },
                                { etiqueta: Idioma.t("stat_ratio_victorias"), valor: Math.round(100 * statsPartidasGanadas / statsPartidasJugadas) + "%" },
                                { etiqueta: Idioma.t("stat_racha_actual"), valor: statsRachaActual + "" },
                                { etiqueta: Idioma.t("stat_mejor_racha"), valor: statsRachaMaxima + "" },
                                { etiqueta: Idioma.t("stat_manos_jugadas"), valor: statsManosJugadas + "" },
                                ...
                                { etiqueta: Idioma.t("stat_mejor_mano"), valor: statsMejorManoFecha > 0
                                      ? comboLocalizado(statsMejorManoNombre) + " (" + formatearFechaCorta(statsMejorManoFecha) + ")"
                                      : "—" }
                            ]
                            delegate: Row {
                                required property var modelData
                    ...
                    Column {
                        ...
                        Repeater {
                            model: [
                                { etiqueta: Idioma.t("combo_carta_alta"), valor: statsVecesCartaAlta },
                                { etiqueta: Idioma.t("combo_pareja"), valor: statsVecesPareja },
                                { etiqueta: Idioma.t("combo_doble_pareja"), valor: statsVecesDoblePareja },
                                { etiqueta: Idioma.t("combo_trio"), valor: statsVecesTrio },
                                { etiqueta: Idioma.t("combo_escalera"), valor: statsVecesEscalera },
                                { etiqueta: Idioma.t("combo_color"), valor: statsVecesColor },
                                { etiqueta: Idioma.t("combo_full_house"), valor: statsVecesFullHouse },
                                { etiqueta: Idioma.t("combo_poker"), valor: statsVecesPoker },
                                { etiqueta: Idioma.t("combo_escalera_color"), valor: statsVecesEscaleraColor },
                                { etiqueta: Idioma.t("combo_escalera_real"), valor: statsVecesEscaleraReal }
                            ]
                            delegate: Row {
                                required property var modelData
                                width: parent.width
                ...
                Text {
                    ...
                    visible: statsPartidasJugadas === 0
                    wrapMode: Text.WordWrap
                    text: Idioma.t("vacio_partidas_oficiales")
                    color: Tema.colorTextoTenue
                    font.pixelSize: 12 * Tema.escala
                }
                ...
                Row {
                    ...
                    Column {
                        ...
                        spacing: 8 * Tema.escala
                        BotonContorno {
                            text: formularioUsernameAbierto ? Idioma.t("boton_cancelar") : Idioma.t("boton_cambiar_username")
                            onClicked: {
                        ...
                        Column {
                            ...
                            CampoTexto {
                                ...
                                width: parent.width
                                font.pixelSize: 13 * Tema.escala
                                placeholderText: (activeFocus || text.length > 0) ? "" : Idioma.t("placeholder_nuevo_username")
                    ...
                    Column {
                        ...
                        spacing: 8 * Tema.escala
                        BotonContorno {
                            text: formularioPasswordAbierto ? Idioma.t("boton_cancelar") : Idioma.t("boton_cambiar_password")
                            onClicked: {
                            ...
                            BotonRelleno {
                                ...
                                onClicked: {
                                    if (campoPasswordActualCuenta.text.length === 0) {
                                        mensajeErrorLogin = "error_falta_password_actual";
                                        return;
                                    }
                                    if (campoPasswordNuevaCuenta.text.length < 8) {
                                        mensajeErrorLogin = "error_password_nueva_corta";
                                        return;
                                    }
                                    mensajeErrorLogin = "";
            ...
            Text {
                ...
                horizontalAlignment: Text.AlignHCenter
                wrapMode: Text.WordWrap
                text: Idioma.t("progreso_invitado_aviso")
                color: Tema.colorTextoTenue
                font.pixelSize: 13 * Tema.escala
            }
                ...
                Row {
                    ...
                    Text {
                        ...
                        text: Idioma.tf("etiqueta_xp_siguiente_nivel",
                                        [progresoNivelActual.xpEnNivel, progresoNivelActual.xpParaSiguiente])
                        color: Tema.colorTextoTenue
                        font.pixelSize: 12 * Tema.escala
                    }
                }
                    ...
                    Column {
                        ...
                        Text {
                            text: Idioma.tf("etiqueta_marco_actual",
                                      [nombreMarco(Tema.marcoPorPartidasGanadas(statsPartidasGanadas, statsTieneMarcoBasico))])
                            color: Tema.colorTexto
                            font.bold: true
                            font.pixelSize: 15 * Tema.escala
                        }
                        ...
                        Column {
                            ...
                            Row {
                                ...
                                Text {
                                    ...
                                    text: Idioma.tf("etiqueta_siguiente_marco",
                                              [nombreProximoMarco(statsPartidasGanadas, statsTieneMarcoBasico)])
                                    color: Tema.colorTextoTenue
                                    font.pixelSize: 12 * Tema.escala
                                }
                                Text {
                                    ...
                                    text: Idioma.tf("etiqueta_victorias_progreso",
                                              [statsPartidasGanadas, proximoUmbralMarco(statsPartidasGanadas, statsTieneMarcoBasico)])
                                    color: Tema.colorTexto
                                    font.bold: true
                                    font.pixelSize: 12 * Tema.escala
                                }
                            ...
                            delegate: Item {
                                ...
                                Row {
                                    ...
                                    Text {
                                        ...
                                        width: 90 * Tema.escala
                                        horizontalAlignment: Text.AlignRight
                                        text: filaMarcoRoadmap.conseguido ? Idioma.t("texto_conseguido") : (statsPartidasGanadas + " / " + filaMarcoRoadmap.modelData.umbral)
            ...
            Text {
                ...
                horizontalAlignment: Text.AlignHCenter
                wrapMode: Text.WordWrap
                text: Idioma.t("logros_invitado_aviso")
                color: Tema.colorTextoTenue
                font.pixelSize: 13 * Tema.escala
            }
            ...
            Column {
                ...
                Text {
                    text: Idioma.tf("etiqueta_logros_desbloqueados", [logrosDesbloqueados(), logrosModel.count])
                    color: Tema.colorTextoTenue
                    font.pixelSize: 12 * Tema.escala
                }
                Text {
                ...
                Repeater {
                    ...
                    delegate: Rectangle {
                        ...
                        border.width: porReclamar ? 2 : 1
                        border.color: porReclamar ? Tema.colorAccent
                                      : (desbloqueado ? colorRareza(rareza) : Qt.rgba(1, 1, 1, 0.12))
                        gradient: Gradient {
                            GradientStop { position: 0.0; color: Qt.lighter(Tema.colorPanel, 1.4) }
                            GradientStop { position: 1.0; color: Tema.colorPanel }
                        }
                        ...
                        Row {
                            ...
                            Rectangle {
                                ...
                                Text {
                                    anchors.centerIn: parent
                                    text: tarjetaLogro.porReclamar ? "!" : (tarjetaLogro.desbloqueado ? "✓" : tarjetaLogro.rareza.charAt(0).toUpperCase())
                            ...
                            Column {
                                ...
                                width: parent.width - 40 * Tema.escala - 14 * Tema.escala - 96 * Tema.escala - 14 * Tema.escala
                                spacing: 4 * Tema.escala
                                ...
                                Text {
                                    visible: tarjetaLogro.porReclamar
                                    text: Idioma.tf("etiqueta_logro_por_reclamar", [tarjetaLogro.xpRecompensa])
                                    color: Tema.colorAccent
                                    font.bold: true
                                    font.pixelSize: 11 * Tema.escala
                                }
                                Text {
                                    visible: tarjetaLogro.desbloqueado === 1 && !tarjetaLogro.porReclamar
                                    text: Idioma.tf("etiqueta_logro_desbloqueado_fecha",
                                              [Qt.formatDate(new Date(tarjetaLogro.desbloqueadoEn * 1000), "d MMM yyyy"),
                                               tarjetaLogro.xpRecompensa])
                                    color: colorRareza(tarjetaLogro.rareza)
                                    font.pixelSize: 10 * Tema.escala
                                }
                            }
                            ...
                            BotonRelleno {
                                anchors.verticalCenter: parent.verticalCenter
                                width: 96 * Tema.escala
                                visible: tarjetaLogro.porReclamar
                                text: Idioma.t("boton_reclamar_logro")
                                onClicked: redcliente.reclamarLogro(servidorHost, servidorPuerto, tokenSesion, tarjetaLogro.codigo)
                            }
                            Text {
                                anchors.verticalCenter: parent.verticalCenter
                                width: 96 * Tema.escala
            ...
            Text {
                ...
                horizontalAlignment: Text.AlignHCenter
                wrapMode: Text.WordWrap
                text: Idioma.t("personalizar_invitado_aviso")
                color: Tema.colorTextoTenue
                font.pixelSize: 13 * Tema.escala
            }
                                ...
                                Rectangle {
                                    ...
                                    Column {
                                        ...
                                        BotonContorno {
                                            ...
                                            onClicked: esTapete ? abrirPopupTapete(celdaPersonalizar.codigo)
                                                : equiparConAcabado(celdaPersonalizar.categoria,
                                                    celdaPersonalizar.equipado === 1 ? "" : celdaPersonalizar.codigo)
                                        }
                                        // Decoraciones laterales: un solo botón que abre la ventana
                                        // de lado (izquierda/derecha/ambos) y material -- antes eran
                                        // dos botones sueltos y el material ni se preguntaba.
                                        BotonContorno {
                                            readonly property bool enIzq: redcliente.loadoutMarco.decoracionLateral1 === celdaPersonalizar.codigo
                                            readonly property bool enDer: redcliente.loadoutMarco.decoracionLateral2 === celdaPersonalizar.codigo
                                            visible: celdaPersonalizar.categoria === "decoracion_lateral"
                                            width: parent.width
                                            text: (enIzq || enDer) ? Idioma.t("boton_modificar") : Idioma.t("boton_equipar")
                                            onClicked: popupAcabado.abrirLateral(celdaPersonalizar.codigo, celdaPersonalizar.nombre,
                ...
                Item {
                    ...
                    Column {
                        ...
                        CampoTexto {
                            ...
                            width: parent.width
                            placeholderText: (activeFocus || text.length > 0) ? "" : Idioma.t("placeholder_buscar_tienda")
                            font.pixelSize: 13 * Tema.escala
                            onTextChanged: {
                                busquedaTienda = text;
                                reordenarTienda();
                                ...
                                Column {
                                    ...
                                    Row {
                                        ...
                                        Row {
                                            ...
                                            Text {
                                                anchors.verticalCenter: parent.verticalCenter
                                                text: Idioma.tf("etiqueta_nivel", [celdaTienda.nivelMinimo])
                                                color: Tema.colorTextoTenue
                                                font.pixelSize: 11 * Tema.escala
                                            }
                                        }
                                    ...
                                    Text {
                                        ...
                                        text: celdaTienda.logroNombre !== ""
                                              ? Idioma.tf("etiqueta_logro", [celdaTienda.logroNombre])
    ...
    Rectangle {
        ...
        Text {
            ...
            horizontalAlignment: Text.AlignHCenter
            wrapMode: Text.WordWrap
            text: Idioma.t("texto_ventana_pequena")
            color: Tema.colorTextoSobreOscuro
            font.pixelSize: 16 * Tema.escala
        }
    }
    ...
    Rectangle {
        ...
        Column {
            ...
            Text {
                anchors.horizontalCenter: parent.horizontalCenter
                text: Idioma.tf("etiqueta_segundos_restantes", [segundosReconexion])
    ...
    // fin de mano viven en la barra inferior, en el hueco de los botones de turno.)
    ...
    Rectangle {
        ...
        Column {
            ...
            // Los seis temas ya no son una lista fija aquí (ocupaban media
            // barra): un botón abre una ventana flotante con todos ellos.
            Column {
                width: parent.width
                spacing: 10 * Tema.escala
            ...
            Column {
                ...
                Repeater {
                    ...
                    model: [
                        { etiqueta: Idioma.t("ajustes_ciega_actual"), valor: Math.round(ciegaActual / 2) + " / " + ciegaActual },
                        { etiqueta: Idioma.t("ajustes_mano"), valor: manoActual + " / " + objetivoManos },
                        { etiqueta: Idioma.t("ajustes_tipo_limite"),
                          valor: [Idioma.t("limite_sin_limite"), Idioma.t("limite_limite_bote"), Idioma.t("limite_limite_fijo")][tipoLimiteActual] || "—" },
                        { etiqueta: Idioma.t("ajustes_permite_recompra"), valor: permitirRecompraActual ? Idioma.t("texto_si") : Idioma.t("texto_no") },
                        { etiqueta: Idioma.t("ajustes_rellena_bots"), valor: rellenarConBotsActual ? Idioma.t("texto_si") : Idioma.t("texto_no") },
                        { etiqueta: Idioma.t("ajustes_pregunta_extender"), valor: preguntarExtensionActual ? Idioma.t("texto_si") : Idioma.t("texto_no") }
                    ].concat(codigoSalaPropia !== ""
                        ? [{ etiqueta: Idioma.t("ajustes_codigo_invitacion"), valor: codigoSalaPropia }]
                        : [])
                    delegate: Row {
                        required property var modelData
                        width: parent.width
            ...
            Column {
                ...
                BotonContorno {
                    width: parent.width
                    text: Idioma.t("boton_descargar_version")
