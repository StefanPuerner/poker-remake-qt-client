...
Column {
    ...
    Item {
        ...
        // Abanico de mini-cartas boca abajo -- SOLO rivales, no la propia
        // silla (la mano propia ya se ve grande en la barra inferior;
        // duplicarla aquí sería ruido, pedido explícito 2026-09-17: "en
        // escritorio ya son visibles directamente"). Se oculta del todo
        // (no solo atenuada) al retirarse -- mismo criterio que el resto
        // del asiento, y también cuando llega el showdown real (overlay
        // aparte, ver showdownAbierto en Main.qml) porque ese overlay ya
        // es la revelación de verdad, no hace falta ninguna animación de
        // "voltear" aquí.
        //
        // Reutiliza la técnica de "coronaDeCartas" en Avatar.qml (marco
        // de escalera real/póker de ases): cada carta gira sobre su
        // propia base (transformOrigin: Item.Bottom) con un ángulo
        // centrado por índice -- misma fórmula "(index - (N-1)/2) *
        // pasoAngulo", aquí con N=2 fijo. A diferencia de la corona (que
        // reparte las cartas en un arco alrededor del marco), aquí no hay
        // círculo -- las dos comparten el mismo punto de apoyo abajo y
        // solo se desplazan un poco en horizontal, dando el aspecto de
        // "abanico de mano" solapado e inclinado que pidió el usuario en
        // vez de las dos cartas rectas de antes.
        Item {
            id: abanico
            visible: filaNombreYCartas.mostrarCartas
